...
function printVisMap(visMap: number[][]): void {
  console.log(visMap.map(row => row.map(x => x.toString()).join('')).join('\n'))
}

function seenTrees(direction: number[], tree: number): number {
...
function solution(input:string[]): (string|number)[]{
  const forest: number[][] = input.map(x => x.split('').map(t => parseInt(t)))
  const forDim: number[] = [forest[0].length, forest.length]                    // [x, y]
  const visMap: number[][] = forest.map(row => new Array(forDim[0]).fill(1))    // if visable --> 1
  const scenic_scores: number[] = []
  // const sceMap: number[][] = forest.map(row => new Array(forDim[0]).fill(0)) // not usfull, maby for visualisation, or just for fun 
  for(let row: number = 1; row < forDim[1] - 1; row++){                         // start with 1, becouse edges are always visable
    for(let col: number = 1; col < forDim[0] - 1; col++){                       // ends with lenth-1 same reason
      const currentTree: number = forest[row][col]
      const top: number[] = forest.slice(0, row).map(row => row[col])
      const bot: number[] = forest.slice(row + 1).map(row => row[col])
      const lef: number[] = forest[row].slice(0, col)
      const rig: number[] = forest[row].slice(col + 1)
      const isInvisible: boolean = [top, bot, lef, rig].map(dir => dir.some(tree => tree >= currentTree)).every(dir => dir)
      if(isInvisible){
        visMap[row][col] = 0
      } else {
        const seen_top: number = seenTrees(top.reverse(), currentTree)
        const seen_bot: number = seenTrees(bot, currentTree)
        const seen_lef: number = seenTrees(lef.reverse(), currentTree)
        const seen_rig: number = seenTrees(rig, currentTree)
        const scenic_score: number = seen_top * seen_bot * seen_lef * seen_rig
...
function job(input:string): void {
  ...
  const [answer1,answer2]: (string|number)[] = solution(lines)
