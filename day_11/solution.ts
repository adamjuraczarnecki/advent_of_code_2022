...
class Monkey {
  ...
  inspect(item: number, woried: boolean): [boolean, number] {
    const factor: number = this.operation[1] === 'old' ? item : parseInt(this.operation[1])
    let worryLevel = item
    if(this.operation[0] === '+'){
...
function printMonkeys(monkeys: Monkey[]): void {
  console.log(
    monkeys.map((monkey, i) => `Monkey ${i}: `.concat(monkey.items.join(', ')) ).join('\n')
  )
}
function printInspectedItems(monkeys: number[]): void {
  console.log(
    monkeys.map((monkey, i) => `Monkey ${i}: inspected items ${monkey} times`).join('\n')
  )
}

function solution1(input:string[], woried: boolean = false, rounds: number = 20): (string|number){
  const monkeys: Monkey[] = input.map(x => new Monkey(x))
  const monkeysLen: number = monkeys.length
  const monkeysOverhead: number = monkeys.map(monkey => monkey.test).reduce((a, x) => a*x) // cap on bignumbers
